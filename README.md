...
- 🎯 **持续按压跟随** — 实时跟踪绿色进度条位置，按住方向键不放直到指针归位，响应更快无超调
...
4. 程序自动循环：
   ...
   - 自动跟随绿色进度条（持续按住 A/D 调整指针位置，进入死区或鱼上钩时松手）
   - 鱼捕获后 → 自动点击关闭结算界面
   - 回到第一步，等待下一次钓鱼
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `kMatchThreshold` | 0.80 | 模板匹配置信度阈值（0~1，`main.cpp`） |
| `kDeadZone` | 10 | 指针到目标距离小于此像素数松手（`follower.h`） |
...
| 跟随循环间隔 | ~0.03s | 持续按压检测频率（`main.cpp`） |
...
- **旧方案：** 每轮 `PressFor`（按下→计算时长→松开），松手期间存在跟随真空期，容易超调
- **新方案：** 只要指针在死区外就**持续按住**对应方向键不放，仅在以下条件松手：
  1. 指针进入死区（`|target - cursor| < 10`）
  2. 检测不到金色指针（无法确定当前位置）
  3. 两个绿色矩形都缺失（无法确定目标位置，鱼可能已上钩）
- 方向变化时自动先松旧键再按新键，避免双键同时按下
- 检测频率 ~33Hz（30ms 间隔），比断续按键响应更快
